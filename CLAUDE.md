...
**Theme system:** Custom colorscheme in `colors/custom.lua` that calls `lua/theme.lua`. Supports light (`onelight`) and dark (`vague`) palettes from `lua/palette/`. Plugins register extra highlights via `theme.add_highlights(fn)` — the callback returns a scope name and a table with palette-specific highlight functions for `vague` and `onedark` (onelight reuses onedark's palette).
